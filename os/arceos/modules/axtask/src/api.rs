...
const K3_CLUSTER_CPU_COUNT: usize = 8;
#[cfg(feature = "k3_com260kit")]
const K3_CPU_COUNT: usize = K3_CLUSTER_CPU_COUNT * 2;
...
/// Returns the spacemit K3 chip cluster-local default affinity mask for a newly created task.
///
/// A task created by another task inherits the creator CPU's K3 cluster: CPUs
/// 0 through 7 and CPUs 8 through 15 are separate default scheduling domains.
/// Bootstrap code has no current task and retains the normal all-CPU default.
#[cfg(feature = "k3_com260kit")]
pub(crate) fn cpu_mask_onk3() -> AxCpuMask {
    if current_may_uninit().is_none() {
        return cpu_mask_full();
    }

    let _guard = NoPreemptIrqSave::new();
    let Some(cpu_range) = k3_cluster_cpu_range(ax_hal::percpu::this_cpu_id(), ax_hal::cpu_num())
    else {
        return cpu_mask_full();
    };
...
#[cfg(all(test, feature = "k3_com260kit"))]
mod k3_cpu_mask_tests {
    use super::k3_cluster_cpu_range;
    ...
    #[test]
    fn incomplete_or_unknown_k3_topology_does_not_select_an_empty_cluster() {
        assert_eq!(k3_cluster_cpu_range(8, 12), Some(8..12));
        assert_eq!(k3_cluster_cpu_range(8, 8), None);
        assert_eq!(k3_cluster_cpu_range(16, 16), None);
    }
}
