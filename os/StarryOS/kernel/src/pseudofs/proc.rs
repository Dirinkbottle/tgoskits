...
fn write_set_ai_thread_file(data: &[u8]) -> VfsResult<()> {
    ...
    let required_cpu_num = AI_CORE_END as usize + 1;
    if cpu_num < required_cpu_num {
        error!(
            "[set_ai_thread] cpu_num={} cannot cover CPU range 0-{}, required at least {}; \
             operation ignored",
            cpu_num, AI_CORE_END, required_cpu_num
        );
        return Ok(());
    }
