...
#[cfg(feature = "k3_com260kit")]
static AI_THREAD_SET: SpinLock<BTreeMap<u32, Vec<u32>>> = SpinLock::new(BTreeMap::new());
...
fn pick_least_loaded_ai_core(map: &BTreeMap<u32, Vec<u32>>, exclude: Option<u32>) -> u32 {
    (AI_CORE_START..=AI_CORE_END)
        .filter(|core| exclude.is_none_or(|excluded| *core != excluded))
        .min_by_key(|core| map.get(core).map_or(0, Vec::len))
        .unwrap_or_else(|| {
            (AI_CORE_START..=AI_CORE_END)
                .min_by_key(|core| map.get(core).map_or(0, Vec::len))
...
fn prune_ai_thread_set(map: &mut BTreeMap<u32, Vec<u32>>) {
    for tids in map.values_mut() {
        tids.retain(|tid| {
            PidNumber::try_from(*tid)
                .map(TidNumber::from)
                .and_then(get_task_by_number)
                .is_ok()
        });
    }
...
fn render_set_ai_thread_file() -> String {
    ...
    writeln!(
        output,
        "Write '1' or 'self' to mark current thread as AI thread.\nWrite '0' or '-self' to unmark current thread.\n"
    )
    .unwrap();

    let map = AI_THREAD_SET.lock();
    let total = map.values().map(Vec::len).sum::<usize>();
    if total == 0 {
    ...
    for core in AI_CORE_START..=AI_CORE_END {
        if let Some(tids) = map.get(&core) {
            for &tid in tids {
                let (pid, name) = PidNumber::try_from(tid)
                    .map(TidNumber::from)
                    .and_then(get_task_by_number)
                    .ok()
                    .map(|task| {
                        ...
                        let exe = thread.proc_data.exe_path();
                        let name = exe
                            .rsplit_once('/')
                            .map(|(_, base)| base)
                ...
                writeln!(output, "  core={core:<3} tid={tid:<6} pid={pid:<6} {name}").unwrap();
            }
        }
    }
    output
    ...
    match command {
        SetAiThreadCommand::Mark => {
            let old_core = map
                ...
                .map(|(&core, _)| core);
            for tids in map.values_mut() {
                tids.retain(|&tid| tid != current_tid);
            }
            map.retain(|_, tids| !tids.is_empty());
            let core = pick_least_loaded_ai_core(&map, old_core);
            map.entry(core).or_default().push(current_tid);
            if let Ok(cpu_count) = ax_task::cpu_topology_len() {
                let mut affinity = CpuSet::empty(cpu_count);
                if affinity.insert(CpuId::new(core)) {
                    let _ = ax_task::set_current_thread_affinity(affinity);
                }
            }
        }
        SetAiThreadCommand::Unmark => {
            ...
            map.retain(|_, tids| !tids.is_empty());
            let Ok(cpu_count) = ax_task::cpu_topology_len() else {
                return Ok(());
            };
            let mut mask = CpuSet::empty(cpu_count);
            for cpu in 0..AI_CORE_START {
                let _ = mask.insert(CpuId::new(cpu));
            }
            let _ = ax_task::set_current_thread_affinity(mask);
        }
    }
    Ok(())
}
...
fn read_kallsyms() -> KallsymsMapped<'static> {
    ...
    let total_size = KallsymsMapped::check_total_bytes(kallsyms_sec)
        .unwrap_or_else(|error| panic!("Invalid kallsyms format: {error:?}"));
    ...
    .unwrap_or_else(|error| panic!("Failed to create KallsymsMapped: {error:?}"))
}
