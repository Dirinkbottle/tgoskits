...
fn sys_prctl_get_auxv(
    ...
    let auxv = current.as_thread().proc_data.auxv();
    let saved_auxv = saved_auxv_bytes(&auxv)?;
    let copy_len = len.min(saved_auxv.len());
    if copy_len != 0 {
        vm_write_slice(current, address, &saved_auxv[..copy_len])?;
    }
    Ok(saved_auxv.len() as isize)
}

fn saved_auxv_bytes(auxv: &[AuxEntry]) -> StarryResult<[u8; LINUX_SAVED_AUXV_SIZE]> {
...
pub fn sys_prctl(
    ...
    match option {
        ...
        PR_GET_AUXV => {
            return sys_prctl_get_auxv(current, arg2 as *mut u8, arg3, arg4, arg5);
        }
        PR_SET_MM => {
