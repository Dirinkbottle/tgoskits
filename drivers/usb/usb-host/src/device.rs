...
use crate::{
    ...
    driver::{UsbDevice, UsbInterface},
};

#[derive(Clone)]
pub struct DeviceInfo {
    pub(crate) inner: Arc<dyn DeviceInfoOp>,
    interfaces: Vec<Arc<UsbInterface>>,
}
...
impl DeviceInfo {
    pub(crate) fn new(inner: Box<dyn DeviceInfoOp>) -> Self {
        let inner: Arc<dyn DeviceInfoOp> = inner.into();
        let device = Arc::new(UsbDevice::new(inner.descriptor().clone()));
        let interfaces = inner
            .configuration_descriptors()
            .first()
            .into_iter()
            .flat_map(|config| &config.interfaces)
            .filter_map(|interface| interface.alt_settings.first().cloned())
            .map(|descriptor| Arc::new(UsbInterface::new(device.clone(), descriptor)))
            .collect();
    ...
    /// Returns the first enumerated configuration's interfaces and their driver bindings.
    pub fn interfaces(&self) -> &[Arc<UsbInterface>] {
        &self.interfaces
    }
