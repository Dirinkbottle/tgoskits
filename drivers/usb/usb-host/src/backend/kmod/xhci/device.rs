...
impl Device {
    ...
    fn calculate_xhci_interval(
        ...
        match transfer_type {
            EndpointType::Isochronous => match port_speed {
                Speed::High | Speed::SuperSpeed | Speed::SuperSpeedPlus => {
                    Self::exponent_interval(binterval)
                }
                Speed::Full => Self::exponent_interval(binterval).saturating_add(3),
                Speed::Low | Speed::Wireless => Self::frame_interval(binterval),
            },
            EndpointType::Interrupt => match port_speed {
                Speed::High | Speed::SuperSpeed | Speed::SuperSpeedPlus => {
                    Self::exponent_interval(binterval)
                }
                Speed::Low | Speed::Full | Speed::Wireless => Self::frame_interval(binterval),
            },
            _ => default,
        }
    }

    /// Converts the HS/SS isochronous or interrupt exponent encoding to the
    /// xHCI interval exponent. USB uses `2^(bInterval - 1)` while xHCI stores
    /// the exponent directly.
    fn exponent_interval(binterval: u8) -> u8 {
        binterval.clamp(1, 16) - 1
    }

    /// Converts the FS/LS frame-count encoding to the xHCI microframe
    /// exponent. Non-power-of-two intervals are rounded down as Linux does.
    fn frame_interval(binterval: u8) -> u8 {
        let microframes = u32::from(binterval.max(1)).saturating_mul(8);
        (u32::BITS - 1 - microframes.leading_zeros())
            .clamp(3, 10)
            .try_into()
            .unwrap_or(10)
    }
...
#[cfg(test)]
mod tests {
    use usb_if::{descriptor::EndpointType, host::hub::Speed};

    use super::Device;

    #[test]
    fn schedules_high_speed_isoch_every_microframe_when_binterval_is_one() {
        assert_eq!(
            Device::calculate_xhci_interval(Speed::High, 1, EndpointType::Isochronous, 0),
            0
        );
    }

    #[test]
    fn schedules_superspeed_interrupt_at_the_descriptor_exponent() {
        assert_eq!(
            Device::calculate_xhci_interval(Speed::SuperSpeed, 4, EndpointType::Interrupt, 0,),
            3
        );
    }

    #[test]
    fn schedules_full_speed_isochronous_endpoints_in_microframes() {
        assert_eq!(
            Device::calculate_xhci_interval(Speed::Full, 1, EndpointType::Isochronous, 0),
            3
        );
    }

    #[test]
    fn rounds_full_speed_interrupt_intervals_down_to_an_xhci_exponent() {
        assert_eq!(
            Device::calculate_xhci_interval(Speed::Full, 3, EndpointType::Interrupt, 0),
            4
        );
    }
}
