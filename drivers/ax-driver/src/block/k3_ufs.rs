...
impl K3UfsController {
    fn new(host: K3UfsHost, num_blocks: u64, block_size: usize) -> Self {
        let mut info = DeviceInfo::new(num_blocks, block_size);
        info.name = Some("k3-ufs");
        Self {
            host: Some(host),
            info,
            started: false,
...
impl DriverGeneric for K3UfsController {
    fn name(&self) -> &str {
        "k3-ufs"
    }
}

impl BlockController for K3UfsController {
    ...
    fn advance(&mut self, event: ControllerEvent) -> Result<ControllerUpdate, BlkError> {
        match event {
            ...
            ControllerEvent::Watchdog { .. } | ControllerEvent::Shutdown => {
                self.stopped = true;
                Ok(ControllerUpdate::state(ControllerState::Shutdown))
            }
...
/// [`HardwareQueue::advance_register_retry`]. The queue never hands the
/// runtime-prepared DMA backing to the device: the UFS controller copies data
/// through its own DMA bounce buffers, so completed backing is returned
/// through [`PreparedDma::complete_without_device`] after CPU-visible access.
struct K3UfsQueue {
    host: K3UfsHost,
    info: QueueInfo,
    pending: VecDeque<(RequestId, OwnedRequest)>,
    next_id: usize,
}
...
impl K3UfsQueue {
    ...
    fn new(host: K3UfsHost, device: DeviceInfo) -> Self {
        let limits = QueueLimits {
            ...
            dma_alignment: device.logical_block_size,
            dma_length_alignment: device.logical_block_size,
            segment_boundary: None,
            max_inflight: 8,
            max_submit_batch: 8,
            max_blocks_per_request: 256,
            max_segments: 1,
            max_segment_size: device.logical_block_size * 256,
    ...
    /// Executes one staged request and returns its terminal result together
    /// with the completed DMA backing.
    fn execute(&mut self, request: OwnedRequest) -> (Result<(), BlkError>, Option<CompletedDma>) {
        ...
        match op {
            RequestOp::Read | RequestOp::Write => {
                if lba > u32::MAX as u64 {
                    return (
                        Err(BlkError::InvalidBlockIndex(lba)),
                ...
                let data = match data {
                    Some(data) => data,
                    None => return (Err(BlkError::InvalidRequest), None),
                };
                let mut buffer = data.into_cpu_buffer();
                // SAFETY: The UFS controller copies through its own DMA bounce
                // buffers; this backing is never exposed to device DMA, so no
                // device access can race with the CPU reads/writes below.
                let slice = unsafe { buffer.as_mut_slice_cpu() };
                let bytes = block_count as usize * block_size;
                let result = match op {
                    RequestOp::Read => {
                        self.host
                            .scsi_read_10(lba as u32, block_count as u16, slice)
                    }
                    _ => self
                        .host
                        .scsi_write_10(lba as u32, block_count as u16, slice),
                };
                match result {
                    Ok(()) => {
                        if op == RequestOp::Read {
                            // The runtime completes FromDevice backing by
                            // invalidating CPU caches; clean our CPU-written
                            // data first so it survives the invalidation.
                            axklib::dma::op().flush(buffer.cpu_ptr(), bytes);
                        }
                        (
                            Ok(()),
                            Some(buffer.prepare_for_device().complete_without_device()),
                        )
                    }
                    Err(e) => {
                        warn!(
                            "[k3-ufs] {} (lba={}, {} bytes) failed: {}",
                            if op == RequestOp::Read {
                                "READ_10"
                            } else {
                                "WRITE_10"
...
impl HardwareQueue for K3UfsQueue {
    fn id(&self) -> usize {
        0
    }
    ...
    fn advance_register_retry(&mut self, sink: &mut dyn CompletionSink) -> Result<(), BlkError> {
        while let Some((id, request)) = self.pending.pop_front() {
            let (result, data) = self.execute(request);
    ...
    fn shutdown(&mut self, sink: &mut dyn CompletionSink) -> Result<(), BlkError> {
        while let Some((id, request)) = self.pending.pop_front() {
            let data = request.data.map(PreparedDma::complete_without_device);
