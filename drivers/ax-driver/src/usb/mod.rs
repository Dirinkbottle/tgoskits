...
#[path = "usb-serial/mod.rs"]
mod usb_serial;
#[cfg(feature = "uvc-video")]
pub mod uvcvideo;
...
impl PlatformUsbHost {
    ...
    fn new_with_root_hub_speed(
        ...
        #[cfg(any(feature = "uvc-video", feature = "usb-serial"))]
        let mut host = host;
        #[cfg(feature = "uvc-video")]
        uvcvideo::register(&mut host);
        #[cfg(feature = "usb-serial")]
        usb_serial::register(&mut host);
        Self {
            name,
            info,
            host,
    ...
    /// Opens every complete UVC device found by the latest USB topology scan.
    #[cfg(feature = "uvc-video")]
    pub async fn probe_uvc_video(
        &mut self,
    ) -> Result<alloc::vec::Vec<uvcvideo::UvcVideoCapture>, crab_usb::err::USBError> {
        uvcvideo::probe(&mut self.host).await
    }

    /// Opens the first complete UVC device found by the latest USB topology scan.
    #[cfg(feature = "uvc-video")]
    pub async fn probe_first_uvc_video(
        &mut self,
    ) -> Result<Option<uvcvideo::UvcVideoCapture>, crab_usb::err::USBError> {
        uvcvideo::probe_first(&mut self.host).await
    }
