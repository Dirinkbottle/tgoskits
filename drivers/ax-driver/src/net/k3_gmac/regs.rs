...
/// MAC RX Queue Control 0（dwmac4.h 0xa0）：RXQ0EN bits[1:0]。
/// 0=disabled, 1=AV, 2=DCB。U-Boot/Linux 均设 2 (DCB)。
pub const GMAC_RXQ_CTRL0: u32 = 0xa0;
pub const GMAC_RXQ0EN_DCB: u32 = 0b10; // bit1:0 = 2 → DCB mode

// ---------------------------------------------------------------------------
// GMAC_CONFIG 位域（dwmac4.h L159-185）
// bit0=RE, bit1=TE, bit9=DCRS, bit13=DM, bit14=FES, bit15=PS,
...
/// GOC 写操作码（已左移到 bit3:2）。来源 stmmac_mdio.c：`MII_GMAC4_WRITE = 1 << GOC_SHIFT`。
pub const MII_GMAC4_WRITE: u32 = 0b01 << MII_GMAC4_GOC_SHIFT; // = 0x4
...
// ---------------------------------------------------------------------------
// MTL 寄存器（dwmac4.h MTL_CHAN_BASE_ADDR = 0xd00，每通道 stride=0x40）
// 对照 U-Boot EQOS_MTL_REGS_BASE = 0xd00。
// ⚠️ 不是 0xc00——错误的 base 会导致 MTL TX 队列从未使能（TXQEN 写入无效地址）。
// ---------------------------------------------------------------------------
...
pub const MTL_OP_MODE_TSF: u32 = 1 << 1; // TX Store-and-Forward（⚠️ BIT1，不是 BIT0——BIT0=FTQ=Flush TX Queue）
pub const MTL_OP_MODE_TXQEN: u32 = 1 << 3; // TX Queue Enable（bit3:2=0b10 → DCB 模式）
...
pub const DMA_SYS_BUS_MODE: u32 = 0x1004; // AXI 总线模式（FB/BLEN/EAME/OSR 都在这里）
...
// DMA_SYS_BUS_MODE 位（dwmac4_dma.h：FB/BLEN/EAME/OSR 都在此寄存器）
pub const DMA_AXI_BLEN8: u32 = 1 << 2;
pub const DMA_AXI_BLEN16: u32 = 1 << 3;
pub const DMA_SYS_BUS_EAME: u32 = 1 << 11; // Enhanced Address Mode Enable（>32 位寻址）
...
/// Programmable Burst Length（DMA_CHAN_TX/RX_CONTROL bit21:16）。
/// 每个 AXI 突发的拍数。PBL=0 时 DMA 引擎不发起突发传输。
pub const DMA_BUS_MODE_PBL_SHIFT: u32 = 16;

/// DMA_CHAN_CONTROL（通道基址 +0x00）位域。
/// PBLX8：8 倍 PBL 乘数（K3 DTS 默认启用，无 snps,no-pbl-x8）。
/// DSL：Descriptor Skip Length（bits[20:18]），每描述符间跳过的总线字数。
///   DmaDesc 填充到 64B（缓存行），DSL=(64-16)/8=6 让 DMA 按 64B 步长读。
pub const DMA_CHAN_CONTROL_PBLX8: u32 = 1 << 16;
pub const DMA_CHAN_CONTROL_DSL_SHIFT: u32 = 18;
pub const DMA_CHAN_CONTROL_DSL_DEFAULT: u32 = 6; // (64-16)/8 = 6 words of 8-byte bus
...
///
/// K3 GMAC 是 DWMAC 5.10a（synopsys_id=0x54 ≥ DWMAC_CORE_4_10=0x41），
/// 必须走 4.10a 版位布局：NIE=BIT15|AIE=BIT14|FBE=BIT12|RIE=BIT6|TIE=BIT0。
/// 来源：dwmac4_dma.h DMA_CHAN_INTR_DEFAULT_MASK_4_10。
pub const DMA_CHAN_INTR_DEFAULT_MASK: u32 = 0xD041;
...
/// DMA_CHAN_CONTROL（通道基址 + 0x00）：PBLX8 等通道级控制。
pub const fn dma_chan_control(chan: u32) -> u32 {
    dma_chan_base(chan)
}
...
/// 编译期检查描述符大小（DmaDesc 填充到 64 字节 = K3 缓存行大小）。
const _: () = {
    assert!(core::mem::size_of::<super::desc::DmaDesc>() == 64);
};
