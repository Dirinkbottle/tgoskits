...
- **RISC-V non-coherent DMA memory attributes**: keep cache-block operations and page-based memory types separate. SpacemiT K3 advertises both Zicbom and Svpbmt, so its OS feature must enable `axplat-dyn/zicbom` for streaming-DMA `cbo.clean` / `cbo.inval` / `cbo.flush` operations and `axplat-dyn/svpbmt` so coherent DMA aliases encode standard PBMT `NC`/`IO` attributes. Do not infer Svpbmt from Zicbom, and do not enable the T-Head MAE feature for K3: it selects T-Head-specific cache instructions and PTE attributes.
