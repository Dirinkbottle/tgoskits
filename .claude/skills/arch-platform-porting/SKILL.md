...
- **RISC-V non-coherent DMA cache maintenance**: keep CPU-family cache extensions separate. SpacemiT K3 uses Zicbom, so enable `axplat-dyn/zicbom` through the K3 OS feature and issue `cbo.clean` / `cbo.inval` / `cbo.flush` over the DMA range. Do not enable the T-Head MAE feature for K3: it selects T-Head-specific cache instructions and PTE attributes.
